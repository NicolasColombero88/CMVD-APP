...
func (r *Repository) GetAllWaybillAdmin(
	...
	if status != "" {
		if status == "activas" || status == "Activas" {
...
func (r *Repository) DeleteWaybill(waybillID primitive.ObjectID) error {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	result, err := waybillsCollection.DeleteOne(ctx, filter)
	if err != nil {
		return fmt.Errorf("error deleting waybill: %w", err)
	}
	if result.DeletedCount == 0 {
		return fmt.Errorf("no waybill found with id: %s", waybillID.Hex())
	}
	return nil
}
