...
export default function FormWaybill({ type = "get" }) {
  ...
  const index = async () => {
    ...
    if (typeof dataSt.shipments_enabled != "undefined") {
      setShipmentsEnabled(dataSt.shipments_enabled);
      if (!dataSt.shipments_enabled) {
        Swal.fire({
          icon: "warning",
          title: "Aviso",
          text: "En este momento no se permite generar guías",
  ...
  const valitateBranch =async () => {
    if(selectCompany.id==""){
      Swal.fire({
        icon: "warning",
        title: "Aviso",
        text: "Seleciona una empresa primero",
        confirmButtonText: "Entendido",
      }).then(() => {
      ...
      <form className="w-full" onSubmit={handleSubmit}>
        <div className="flex flex-wrap -mx-2">
          ...
          <div className="w-full md:w-1/3 px-2 ">
            <SelectHoursRange value={data.hour} set={setSelectHours} required={true} disabled={type == "get"} labelText="Hora de retiro (8hs - 16hs)" />
          </div>
          <div className="w-full md:w-1/4 px-2 ">
            <label
              htmlFor="price"
